...
class UpdatePartInventoryRequest extends FormRequest
{
    /**
    ...
    {
        $part = $this->route('partsInventory');

        // Check permission
        if (!$this->user()->can('inventory.edit')) {
            return false;
        }

        // Branch-level authorization (non-admin can only edit their branch)
        if (!$this->user()->hasRole('admin') && !$this->user()->hasRole('auditor')) {
            if ($part->branch_id !== $this->user()->branch_id) {
                return false;
            }
        }
    ...
    public function rules(): array
    {
        ...
        return [
            // Basic Information (branch_id is NOT updatable)
            ...
            // Barcode/SKU (unique except for current record)
            'barcode' => 'nullable|string|max:100|unique:parts_inventory,barcode,' . $part->id,
            'sku' => 'nullable|string|max:100|unique:parts_inventory,sku,' . $part->id,
        ];
    }
